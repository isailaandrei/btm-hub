-- Owner curation of WhatsApp threads: soft-deactivate irrelevant messages.
--
-- A non-null deactivated_at moves the message out of the contact thread's active
-- view (into a restorable "Removed" area) and excludes it from every path that
-- feeds the admin-AI knowledge base: vector + keyword retrieval, and digest /
-- embedding generation. Fully reversible — no data is deleted.
...
-- Retrieval: exclude deactivated messages immediately, even if already embedded.
...
-- Generation: never digest or embed a deactivated message.
