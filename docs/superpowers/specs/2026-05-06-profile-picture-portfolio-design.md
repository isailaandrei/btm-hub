...
- Authenticated users can read portfolio objects only through signed URLs generated by server code.
- Admins can read portfolios through the same server-generated signed URL path.

Signed URLs:

- Data fetchers return portfolio items with short-lived signed image URLs for display.
- Signed URLs are generated server-side with Supabase Storage `createSignedUrls`.
- Missing/inaccessible objects fail visibly in the UI instead of silently showing fake placeholders.
