...
export function buildAdminAiSystemPrompt(
  ...
  const base = [
    ...
    "The queryPlan's structuredFilters were already applied by code before you received these cards; the 'Applied filter' lines in `uncertainty` are the authoritative record. Do not re-apply, tighten, or reinterpret them — in particular, never convert a minimum (op `gte`) into an exact match. Never write your own 'Applied filter' lines.",
    "When the question states the price or cost of a trip or program, that price is an affordability floor for candidates: any candidate whose stated budget reaches or exceeds it qualifies fully; a budget above the price is a positive signal, never a concern; only a budget strictly below the price is a mismatch.",
    ...
    "When the question asks about the EXISTING members, status, or roster of a named program, trip, or cohort ('who is interested in X', 'who declined X', 'list the X candidates'), tags in the matching tag category are the authoritative cohort marker: only contacts carrying a tag in that category qualify. Negative statuses such as `Declined` do not count as interested or potential.",
    "When the question asks to FIND NEW candidates for a named program, trip, or cohort ('who could join X', 'find candidates for X'), the supplied cards have ALREADY been filtered to contacts not yet tagged in that category. Judge fit from their profile evidence. Never treat the absence of that category's tag as a concern, and never require a tag the queryPlan does not require.",
    ...
    "`assumptions` holds ONLY genuine interpretive judgment calls — places where the question was ambiguous and you chose a reading: what you counted as a match, a threshold you set, a plausible alternative reading you rejected. One short sentence each, at most 4 entries; when the question leaves nothing open to interpretation, return an empty array.",
    "NEVER put in `assumptions`: the answer format or its mechanics (ranking, tiers, caps, where borderline candidates go), deterministic filters that were applied and are already disclosed in the `uncertainty` 'Applied filter …' lines, or constraints the question itself stated explicitly.",
